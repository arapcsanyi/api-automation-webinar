...
describe('Photos', () => {
    describe('Create', () => {
        ...
        it('should add a new photo', () => {
            return chakram.post(api.url('photos'), {
                ...
                expect(photo).to.have.json('data.albumId', 1);
                expect(photo).to.have.json('data.title', 'title');
                expect(photo).to.have.json('data.url', 'url');
                expect(photo).to.have.json('data.thumbnailUrl', 'thumbnailUrl');
                return chakram.wait();
            });
        });
    ...
    describe('Update', () => {
        it('should update existing photo with given data', () => {
            const response = chakram.put(api.url('photos/3'), {
                id: 3,
                albumId: 3,
                title: 'title',
                url: 'url',
            ...
            return response.then(data => {
                const post = chakram.get(api.url('photos/3'));
                expect(post).to.have.json('data', data => {
                    expect(data.id).to.equal(3);
                    expect(data.albumId).to.equal(3);
                    expect(data.title).to.equal('title');
                    expect(data.url).to.equal('url');
                    expect(data.thumbnailUrl).to.equal('thumbnailUrl');
                });
                return chakram.wait();
            });
        });
        ...
        it('should throw error if the photo does not exist', () => {
            const response = chakram.put(api.url('photos/6666'), {
                id: 6666,
                albumId: 'albumId',
                title: 'title',
                url: 'url',
    ...
    describe('Delete', () => {
        it('should delete photo by ID', () => {
            const response = chakram.delete(api.url('photos/10'));
            expect(response).to.have.status(200);
            return response.then(data => {
                const photo = chakram.get(api.url('photos/10'));
        ...
        it('should throw error if the photo does not exist', () => {
            const response = chakram.delete(api.url('photos/6666'));
            expect(response).to.have.status(404);
            return chakram.wait();
        });
    });
